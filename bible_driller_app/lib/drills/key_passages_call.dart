...
import '../models/key_passages_call_model.dart';
import '../models/selection_model.dart';
import '../services/key_passages_call_service.dart';

class KeyPassagesCallDrill extends StatefulWidget {
  const KeyPassagesCallDrill({super.key});

 @override
  State<KeyPassagesCallDrill> createState() => _KeyPassagesCallDrillState();
}

class _KeyPassagesCallDrillState extends State<KeyPassagesCallDrill> {
  List<KeyPassagesCallModel> _originalList = [];
  List<KeyPassagesCallModel> _currentList = [];
  int _currentIndex = 0;
 ...
 Future<void> _loadFilteredData() async {
    ...
    final service = KeyPassagesCallService();
    final allCalls = await service.loadKeyPassageCalls();

    final filtered = allCalls.where((call) =>
        call.color == selection.color).toList();
  ...
  void _toggleRandomization(bool? value) {
    setState(() {
      ...
      final list = List<KeyPassagesCallModel>.from(_originalList);
